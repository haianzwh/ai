# Stage 1: Page Planning -- 第 8 页（共 10 页）

> **【系统级强制指令 / CRITICAL OVERRIDE】**
> 本 prompt 已包含了你在此阶段所需的**全部**任务目标与 Playbook 细则。
> **严格禁止调用工具去读取外层的 `SKILL.md` 或主控全局规则文件！**
>
> 本阶段的唯一目标：产出 `ppt-output/runs/20260716-141627-zizhujiashi/planning/planning8.json`。作为架构师，你需要在这里定下不可逾越的**硬性工程图纸**。请在 `layout_hint`、`focus_zone` 和 `must_avoid` 等字段中施加严密的结构控制。后续的 HTML 阶段和图审环节，都将绝对服从你在此刻定下的框架纪律。
> 若外层 orchestrator 已提供阶段推进协议，则外层协议优先于本 prompt 中的完成信号描述。

这是你为第 8 页执行的**第一阶段核心任务**：策划定骨稿。
你暂时不要写 HTML 代码，全力填好并校验 `ppt-output/runs/20260716-141627-zizhujiashi/planning/planning8.json`。
...
- 策划稿 JSON：`ppt-output/runs/20260716-141627-zizhujiashi/planning/planning8.json`
- Runtime 备份：`ppt-output/runs/20260716-141627-zizhujiashi/runtime/page-planning-output-8.json`
- Validator 报告：`ppt-output/runs/20260716-141627-zizhujiashi/runtime/page-planning-validator-8.json`
...
1. 读取 `ppt-output/runs/20260716-141627-zizhujiashi/outline.txt` 中第 8 页的定义（只关注你这一页），特别提取 `密度下限 / 密度目标 / 密度上限 / 节奏动作 / 信息姿态 / 锚点类型`
...
5. 读取主链已生成的**图片清单快照** `ppt-output/runs/20260716-141627-zizhujiashi/runtime/page-images-8.md`。
6. 如需刷新这份图片清单，再执行：
   ```bash
   python3 $SKILL_DIR/scripts/subagent_logger.py run --log ppt-output/runs/20260716-141627-zizhujiashi/runtime/page-agent-8.log --label planning-refresh-images -- \
...
8. 如需刷新这份菜单快照，再执行：
   ```bash
   python3 $SKILL_DIR/scripts/subagent_logger.py run --log ppt-output/runs/20260716-141627-zizhujiashi/runtime/page-agent-8.log --label planning-refresh-menu -- \
...
10. 将完整 planning 写入 `ppt-output/runs/20260716-141627-zizhujiashi/planning/planning8.json`，并同步备份到 `ppt-output/runs/20260716-141627-zizhujiashi/runtime/page-planning-output-8.json`：
   ```bash
   python3 $SKILL_DIR/scripts/subagent_logger.py run --log ppt-output/runs/20260716-141627-zizhujiashi/runtime/page-agent-8.log --label planning-runtime-copy -- \
     cp ppt-output/runs/20260716-141627-zizhujiashi/planning/planning8.json ppt-output/runs/20260716-141627-zizhujiashi/runtime/page-planning-output-8.json
   ```
11. 自审（必须执行，不得跳过）：
   ```bash
   python3 $SKILL_DIR/scripts/subagent_logger.py run --log ppt-output/runs/20260716-141627-zizhujiashi/runtime/page-agent-8.log --label planning-validator -- \
     python3 $SKILL_DIR/scripts/planning_validator.py $(dirname ppt-output/runs/20260716-141627-zizhujiashi/planning/planning8.json) --refs $SKILL_DIR/references --page 8 --report ppt-output/runs/20260716-141627-zizhujiashi/runtime/page-planning-validator-8.json
   ```
12. 修复所有 ERROR（WARNING 建议修复）。
13. 完成信号：输出 `--- STAGE 1 COMPLETE: ppt-output/runs/20260716-141627-zizhujiashi/planning/planning8.json ---`，然后按外层 orchestrator 协议继续下一阶段
