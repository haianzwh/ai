...
>
> 本阶段的唯一目标：读取用户资料并提炼为结构化素材，写入 `ppt-output/runs/20260716-141627-zizhujiashi/source-brief.txt`。
...
资料路径（可能是目录或文件列表）：`/tmp/opencode/ai-chat/server/uploads/zhongweihan/7d9ce6c191eb.docx`
...
按照 Playbook 的阅读策略与提炼指南执行任务，将结构化提炼结果写入 `ppt-output/runs/20260716-141627-zizhujiashi/source-brief.txt`。完成后只输出阶段完成信号：
`--- STAGE 1 COMPLETE: ppt-output/runs/20260716-141627-zizhujiashi/source-brief.txt ---`
