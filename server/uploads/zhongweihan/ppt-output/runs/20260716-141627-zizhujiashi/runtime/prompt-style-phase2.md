...
> **前置条件**：风格输出阶段已完成，`ppt-output/runs/20260716-141627-zizhujiashi/style.json` 已就绪。
...
- 待审查文件：`ppt-output/runs/20260716-141627-zizhujiashi/style.json`
...
1. 读取 `ppt-output/runs/20260716-141627-zizhujiashi/style.json` 全文（解析 JSON）。
2. 根据上方 Playbook 里的 **6项自审检查清单** 对 JSON 进行逐项严格对账。
3. **发现问题立即修复**（直接用工具改写 `ppt-output/runs/20260716-141627-zizhujiashi/style.json`，不可新建文件）。
4. 最多允许 2 轮自我修补循环。
5. 6 项全过后，发送最终 FINALIZE 信号终结该流程：

```
FINALIZE: 自审通过
- style: ppt-output/runs/20260716-141627-zizhujiashi/style.json
- 自审轮数: N
- 修复发现: [列举你按照要求修复了什么不规范字段，若无填 无]
```
