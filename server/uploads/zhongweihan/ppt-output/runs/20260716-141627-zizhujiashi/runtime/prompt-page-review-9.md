# Stage 3: Page QA & Visual Review -- 第 9 页（共 10 页）

> **【系统级强制指令 / CRITICAL OVERRIDE】**
> 本 prompt 已包含了你在此阶段所需的**全部**任务目标与 Playbook 细则。
> **严格禁止调用工具去读取外层的 `SKILL.md` 或主控全局规则文件！**
>
> **前置条件**：Planning + HTML 阶段已完成，`ppt-output/runs/20260716-141627-zizhujiashi/planning/planning9.json` 和 `ppt-output/runs/20260716-141627-zizhujiashi/slides/slide-9.html` 已就绪。
...
|------|--------|
| 页码 | 9 / 10 |
| HTML 源文件 | `ppt-output/runs/20260716-141627-zizhujiashi/slides/slide-9.html` |
| PNG 截图输出 | `ppt-output/runs/20260716-141627-zizhujiashi/png/slide-9.png` |
| 审查存档目录 | `ppt-output/runs/20260716-141627-zizhujiashi/review` |
| Runtime PNG 备份 | `ppt-output/runs/20260716-141627-zizhujiashi/runtime/page-review-output-9.png` |
| visual_qa 报告 | `ppt-output/runs/20260716-141627-zizhujiashi/runtime/page-review-qa-9.txt` |
| 参考风格 | `ppt-output/runs/20260716-141627-zizhujiashi/style.json` |
| 策划原稿 | `ppt-output/runs/20260716-141627-zizhujiashi/planning/planning9.json` |
...
```bash
# 1a. 截图到最终位置
python3 $SKILL_DIR/scripts/subagent_logger.py run --log ppt-output/runs/20260716-141627-zizhujiashi/runtime/page-agent-9.log --label review-html2png -- \
  python3 $SKILL_DIR/scripts/html2png.py ppt-output/runs/20260716-141627-zizhujiashi/slides/slide-9.html -o $(dirname ppt-output/runs/20260716-141627-zizhujiashi/png/slide-9.png) --scale 0.75

# 1b. 归档到轮次目录（每轮必须，X = 当前轮次编号）
mkdir -p ppt-output/runs/20260716-141627-zizhujiashi/review/roundX
cp ppt-output/runs/20260716-141627-zizhujiashi/png/slide-9.png ppt-output/runs/20260716-141627-zizhujiashi/review/roundX/slide-9.png

# 1c. 同步最新截图到 runtime 备份
cp ppt-output/runs/20260716-141627-zizhujiashi/png/slide-9.png ppt-output/runs/20260716-141627-zizhujiashi/runtime/page-review-output-9.png
```

**Step 2 — 读图 + 3 遍系统扫描**

必须用图像工具**实际观察 PNG**（不得凭代码想象）。
只需要查看本轮的最新截图：使用**当前宿主可用的图像查看能力**打开 `ppt-output/runs/20260716-141627-zizhujiashi/review/roundX/slide-9.png`，逐条确认上轮发现的问题是否真正被修复，切勿再读取上一轮的老图！
...
按 Playbook Part D 的优先级（P0→P1→P2）和修复顺序（内容→结构→颜色→装饰），直接修改 `ppt-output/runs/20260716-141627-zizhujiashi/slides/slide-9.html` 的 HTML/CSS 源码。
...
```bash
python3 $SKILL_DIR/scripts/subagent_logger.py run --log ppt-output/runs/20260716-141627-zizhujiashi/runtime/page-agent-9.log --label review-visual-qa -- \
  python3 $SKILL_DIR/scripts/visual_qa.py ppt-output/runs/20260716-141627-zizhujiashi/png/slide-9.png --planning ppt-output/runs/20260716-141627-zizhujiashi/planning/planning9.json --html ppt-output/runs/20260716-141627-zizhujiashi/slides/slide-9.html --output ppt-output/runs/20260716-141627-zizhujiashi/runtime/page-review-qa-9.txt
```
...
```
FINALIZE:
- planning: ppt-output/runs/20260716-141627-zizhujiashi/planning/planning9.json
- html: ppt-output/runs/20260716-141627-zizhujiashi/slides/slide-9.html
- png: ppt-output/runs/20260716-141627-zizhujiashi/png/slide-9.png
- 审查轮数: N (最少 2，无上限)
- P0 状态: 全部通过
- P1 状态: 全部通过
