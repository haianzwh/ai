# Stage 2: Page HTML Production -- 第 8 页（共 10 页）

> **【系统级强制指令 / CRITICAL OVERRIDE】**
> 本 prompt 已包含了你在此阶段所需的**全部**任务目标与 Playbook 细则。
> **严格禁止调用工具去读取外层的 `SKILL.md` 或主控全局规则文件！**
>
> **前置条件**：Planning 阶段已完成，`ppt-output/runs/20260716-141627-zizhujiashi/planning/planning8.json` 已就绪。
> 本阶段的唯一目标：基于 planning JSON 产出 `ppt-output/runs/20260716-141627-zizhujiashi/slides/slide-8.html`。完成后发送 FINALIZE 信号。
> 若外层 orchestrator 已提供阶段推进协议，则外层协议优先于本 prompt 中的完成信号描述。

这是你为第 8 页执行的**第二阶段核心任务**：HTML 设计稿生成。
你的策划稿（`ppt-output/runs/20260716-141627-zizhujiashi/planning/planning8.json`）是本阶段的主要输入，严格忠实还原其骨架。
...
|------|--------|
| 页码 | 8 / 10 |
| 策划稿 | `ppt-output/runs/20260716-141627-zizhujiashi/planning/planning8.json` |
| 风格规范 | `ppt-output/runs/20260716-141627-zizhujiashi/style.json` |
| 输出 HTML | `ppt-output/runs/20260716-141627-zizhujiashi/slides/slide-8.html` |
| 图片清单快照 | `ppt-output/runs/20260716-141627-zizhujiashi/runtime/page-images-8.md` |
| 资源正文快照 | `ppt-output/runs/20260716-141627-zizhujiashi/runtime/page-html-resolve-8.md` |
| Runtime HTML 备份 | `ppt-output/runs/20260716-141627-zizhujiashi/runtime/page-html-output-8.html` |
| 运行日志 | `ppt-output/runs/20260716-141627-zizhujiashi/runtime/page-agent-8.log` |
| SKILL 目录 | `$SKILL_DIR` |
...
1. 读取 `ppt-output/runs/20260716-141627-zizhujiashi/planning/planning8.json`，提取完整骨架（`page_type`、`layout_hint`、`density_label`、`density_contract`、`focus_zone`、`negative_space_target`、`cards[].card_id/role/card_type/card_style/headline/body/data_points/chart/image/resource_ref`、`director_command`、`decoration_hints`、`source_guidance`、`resources`、`must_avoid`）
...
3. **必须执行** —— 获取 planning 引用资源的**正文层实现细节**（不能跳过，里面有组件级 CSS 参数和骨架建议），并把结果备份到 runtime：
   ```bash
   python3 $SKILL_DIR/scripts/subagent_logger.py run --log ppt-output/runs/20260716-141627-zizhujiashi/runtime/page-agent-8.log --label html-resolve-resources -- \
     python3 $SKILL_DIR/scripts/resource_loader.py resolve --refs-dir $SKILL_DIR/references --planning ppt-output/runs/20260716-141627-zizhujiashi/planning/planning8.json --output ppt-output/runs/20260716-141627-zizhujiashi/runtime/page-html-resolve-8.md
   ```
   然后读取 `ppt-output/runs/20260716-141627-zizhujiashi/runtime/page-html-resolve-8.md`。resolve 输出的组件正文是必须严格遵从的起点。你是最严格的设计执行官 -- 在保证 1280x720 画布物理红线的前提下，结合 `page_goal` 和 `director_command`，运用高精度的代码将策划稿在 DOM 树中毫无妥协地还原。图审只会挑出你的瑕疵，不会容忍你重构骨架。
4. 核对图片素材，必要时刷新图片清单快照：
   ```bash
   python3 $SKILL_DIR/scripts/subagent_logger.py run --log ppt-output/runs/20260716-141627-zizhujiashi/runtime/page-agent-8.log --label html-refresh-images -- \
     python3 $SKILL_DIR/scripts/resource_loader.py images --images-dir ppt-output/runs/20260716-141627-zizhujiashi/images --output ppt-output/runs/20260716-141627-zizhujiashi/runtime/page-images-8.md
   ```
   然后读取 `ppt-output/runs/20260716-141627-zizhujiashi/runtime/page-images-8.md`，确认 `image.source_hint` 路径可访问。
...
12. 将完整 HTML 写入 `ppt-output/runs/20260716-141627-zizhujiashi/slides/slide-8.html`，并同步备份到 `ppt-output/runs/20260716-141627-zizhujiashi/runtime/page-html-output-8.html`
    > **🔴 绝对红线警告 🔴**
    > `ppt-output/runs/20260716-141627-zizhujiashi/slides/slide-8.html` 必须是 100% 纯净的 HTML 代码！
    ...
    ```bash
    python3 $SKILL_DIR/scripts/subagent_logger.py run --log ppt-output/runs/20260716-141627-zizhujiashi/runtime/page-agent-8.log --label html-runtime-copy -- \
      cp ppt-output/runs/20260716-141627-zizhujiashi/slides/slide-8.html ppt-output/runs/20260716-141627-zizhujiashi/runtime/page-html-output-8.html
    ```
13. 完成信号：输出 `--- STAGE 2 COMPLETE: ppt-output/runs/20260716-141627-zizhujiashi/slides/slide-8.html ---`，然后按外层 orchestrator 协议继续下一阶段
